...
<BlogFigure name="accessible-web-design-forms" alt="Anatomy of a form field: label, help text, an input with a visible focus ring, an error line marked with an icon, and a large touch target, top to bottom." caption="The order matters: the label stays above the field, help text is read before the input, and the error sits right below it, never signalled by colour alone." />
