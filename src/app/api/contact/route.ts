...
export async function POST(request: Request) {
  ...
  // Sablonlar: src/lib/email (onizleme: /api/email-preview, yalnizca dev).
  const notification = renderContactNotification({ name, email, phone, message, locale });
  ...
  if (CAN_AUTO_REPLY) {
    const reply = renderContactAutoReply({ name, email, phone, message, locale });
    try {
      await resend.emails.send({
        ...
        html: reply.html,
        text: reply.text,
        // "Bu e-postayi yanitlayin" diyoruz; yanit gonderici adresine degil gelen kutusuna dussun.
        replyTo: NOTIFY_TO,
      });
    } catch (error) {
