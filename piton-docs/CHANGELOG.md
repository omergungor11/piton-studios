...
- **Kart görselleri**: 9 kartın hepsinde hizmet sayfalarındaki SVG sahnesi (`PricingArt` → `SERVICE_ART`), kartın üstünde 16:10 `.sv-stage` şeridi olarak. Alt metinler mevcut `serviceVisuals.<slug>.alt` çevirilerinden — fiyat sayfasına yeni metin yazılmadı. Eşleme: şablon→web tasarım, kurumsal→özel yazılım, e-ticaret, web uygulama, mobil uygulama, AI→otomasyon, SEO & GEO / Google Ads / Meta kendi sahneleri.
- CSS: tek kalan 5. kart iki sütunu kaplıyor (`.pricing-card--wide`; ≥1001px'te sahne solda, metin sağda iki sütunlu içerik listesiyle), `.pricing-growth-grid` 3 sütun / mobilde tek, AI bölümünde sahne + liste sağ sütunda (`.pricing-ai-side`). Build 841 sayfa, typecheck + lint + `content:check` temiz.
